...
class Price3DRegressionManager(BaseModel):
    DISCOVERY_FOLDS = [
    ...
    def __init__(
        self,
        data_path: str = PRICE_3D_FEATURES_FILE,
        model_dir: str = MODELS_PRICE_3D_DIR,
        holdout_root: str = HOLDOUT_PRICE_3D_DIR,
        ...
        self.device = DEVICE
        self.feature_builder = Price3DFeatureBuilder(
    ...
    def load_and_preprocess(self, force_refresh_features: bool = False) -> pd.DataFrame:
        ...
        return load_and_preprocess_price_features(
            ...
            missing_date_message="Price3D features file is missing a Date column.",
        )
    ...
    def _build_sequence_payload(self, df: pd.DataFrame, asset: str, feature_cols: List[str], window_size: int) -> Dict[str, np.ndarray]:
        ...
        return build_price_sequence_payload(
            ...
            centered_targets=df[f"CenteredNormPrice_{asset}_3d"].values,
            current_prices=df[asset].values,
            future_prices=df[f"FuturePrice_{asset}_3d"].values,
            mape_valid=df[f"MAPEValid_{asset}_3d"].values.astype(bool),
    ...
    def _resolve_default_metrics_dir(self) -> str:
        ...
        return resolve_default_metrics_dir(
            ...
            metrics_filename="price3d_metrics.json",
