...
class Price1MRegressionManager(BaseModel):
    DISCOVERY_FOLDS = [
    ...
    MODEL_CONFIGS = {
        "nlinear": {"window_size": 40, "batch_size": 256, "lr": 1e-3, "dropout": 0.0},
        "patchtst": {
            "window_size": 80,
            "batch_size": 128,
            "lr": 4e-4,
    ...
    def __init__(
        self,
        data_path: str = PRICE_1M_FEATURES_FILE,
        model_dir: str = MODELS_PRICE_1M_DIR,
        holdout_root: str = HOLDOUT_PRICE_1M_DIR,
        ...
        self.device = DEVICE
        self.feature_builder = Price1MFeatureBuilder(
    ...
    def load_and_preprocess(self, force_refresh_features: bool = False) -> pd.DataFrame:
        ...
        return load_and_preprocess_price_features(
            ...
            missing_date_message="Price1M features file is missing a Date column.",
        )
    ...
    def _build_sequence_payload(self, df: pd.DataFrame, asset: str, feature_cols: List[str], window_size: int) -> Dict[str, np.ndarray]:
        ...
        return build_price_sequence_payload(
            ...
            centered_targets=df[f"CenteredNormPrice_{asset}_1m"].values,
            current_prices=df[asset].values,
            future_prices=df[f"FuturePrice_{asset}_1m"].values,
            mape_valid=df[f"MAPEValid_{asset}_1m"].values.astype(bool),
    ...
    def select_champions_from_metrics(self, metrics: List[Dict[str, object]], holdout_year: int) -> Dict[str, object]:
        ...
        for asset in self.assets:
            ...
            for item in asset_metrics:
                holdout_mape = item["holdout_metrics"].get("mape")
                if holdout_mape is None or baseline_mape is None:
                    continue
                ...
                passes = improvement_absolute >= self.PROMOTION_ABSOLUTE_THRESHOLD or (
                    improvement_relative is not None and improvement_relative >= self.PROMOTION_RELATIVE_THRESHOLD
                )
                if passes:
                    promoted_candidates.append((item, improvement_absolute, improvement_relative))

            if promoted_candidates:
                ranked = sorted(
                    promoted_candidates,
                    key=lambda row: (
                        self._sort_nan(row[0]["holdout_metrics"].get("mape")),
                        -self._sort_nan(row[1]),
                        self._sort_nan(row[0]["holdout_metrics"].get("latency_ms")),
                    ),
                )
                best = ranked[0][0]
            else:
    ...
    def _resolve_default_metrics_dir(self) -> str:
        ...
        return resolve_default_metrics_dir(
            ...
            metrics_filename="price1m_metrics.json",
