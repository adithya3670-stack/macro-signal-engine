...
from config.settings import ASSETS, MASTER_DATA_FILE, PRICE_1W_FEATURES_FILE


class Price1WFeatureBuilder:
    """Builds and caches a dedicated feature table for 1-week price forecasting."""

    def __init__(
        self,
        master_data_path: str = MASTER_DATA_FILE,
        output_path: str = PRICE_1W_FEATURES_FILE,
    ...
    def _add_short_horizon_price_features(self, df: pd.DataFrame) -> pd.DataFrame:
        if not isinstance(df.index, pd.DatetimeIndex):
            raise ValueError("Price1WFeatureBuilder requires a DatetimeIndex.")
    ...
    def _add_targets(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for asset in self.assets:
            ...
            new_cols[f"FuturePrice_{asset}_1w"] = future_price
            new_cols[f"NormPrice_{asset}_1w"] = norm_price
            new_cols[f"CenteredNormPrice_{asset}_1w"] = norm_price - 1.0
            new_cols[f"MAPEValid_{asset}_1w"] = mape_valid
