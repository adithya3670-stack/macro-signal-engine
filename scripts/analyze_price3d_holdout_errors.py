...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Slice 3D price holdout errors by regime, trend, volatility, and calendar buckets.")
...
def main() -> None:
    ...
    payload = manager._create_sequences(
        ...
        centered_targets=df[f"CenteredNormPrice_{args.asset}_3d"].values,
        current_prices=df[args.asset].values,
        future_prices=df[f"FuturePrice_{args.asset}_3d"].values,
        mape_valid=df[f"MAPEValid_{args.asset}_3d"].values.astype(bool),
