...
from scripts.run_price1m_focused_experiment import compute_moving_average_baselines
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a targeted 1M search for monthly price MAPE improvements.")
    ...
    parser.add_argument(
        "--output-root",
        default=os.path.join("models", "holdout_price_1m_experiments", "round_search"),
        help="Directory root for 1M search artifacts.",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional list of experiment names to run. Defaults to the full 1M search set.",
    )
    return parser.parse_args()


def compact_registry(manager: Price1MRegressionManager, df, default_registry: Dict[str, List[str]]) -> Dict[str, Dict[str, List[str]]]:
...
def score_entry(entry: Dict[str, object], ma_best: Dict[str, object]) -> Dict[str, object]:
    ...
    passes_promotion = bool(
        ...
        and (
            improvement_absolute >= Price1MRegressionManager.PROMOTION_ABSOLUTE_THRESHOLD
            or (improvement_relative is not None and improvement_relative >= Price1MRegressionManager.PROMOTION_RELATIVE_THRESHOLD)
        )
    )
    return {
...
def main() -> None:
    ...
    print(f"Running {total} 1M search experiments with max {args.epochs} epochs.")
    ...
    metrics = [row["entry"] for row in results]
    ...
    for asset, row in best_by_asset.items():
        match = next((item for item in results if item["experiment_name"] == row["experiment_name"]), None)
        if match:
            selected_registry[asset] = match["feature_cols"]
    ...
    with open(registry_path, "w") as handle:
        json.dump(selected_registry, handle, indent=2)
    ...
    print(f"\nSaved 1M search artifacts to {holdout_dir}")
