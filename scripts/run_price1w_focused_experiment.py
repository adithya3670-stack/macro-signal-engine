...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a focused 1W price experiment on the best current model family per asset.")
    ...
    parser.add_argument(
        "--output-root",
        default=os.path.join("models", "holdout_price_1w_experiments", "focused_run"),
...
def compute_moving_average_baselines(
    ...
    future_prices = df[f"FuturePrice_{asset}_1w"]
    mape_valid = df[f"MAPEValid_{asset}_1w"].astype(bool)
...
def main() -> None:
    ...
    print(f"Running focused 1W price experiment for {len(selected_assets)} assets with max {args.epochs} epochs.")
    ...
    metrics_path = os.path.join(holdout_dir, "price1w_metrics.json")
    champions_path = os.path.join(holdout_dir, "price1w_champions.json")
    registry_path = os.path.join(holdout_dir, "price1w_feature_registry.json")
    ma_path = os.path.join(holdout_dir, "price1w_moving_average_baselines.json")
    leaderboard_path = os.path.join(holdout_dir, "price1w_leaderboard.json")
    experiment_path = os.path.join(holdout_dir, "price1w_focused_experiment.json")
