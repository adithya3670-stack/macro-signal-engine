...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a focused 3D price experiment on the best current model family per asset.")
    ...
    parser.add_argument(
        "--output-root",
        default=os.path.join("models", "holdout_price_3d_experiments", "focused_run"),
...
def compute_moving_average_baselines(
    manager: Price3DRegressionManager,
    df,
    asset: str,
    ...
    future_prices = df[f"FuturePrice_{asset}_3d"]
    mape_valid = df[f"MAPEValid_{asset}_3d"].astype(bool)
...
def main() -> None:
    ...
    print(f"Running focused 3D price experiment for {len(selected_assets)} assets with max {args.epochs} epochs.")
    ...
    metrics_path = os.path.join(holdout_dir, "price3d_metrics.json")
    champions_path = os.path.join(holdout_dir, "price3d_champions.json")
    registry_path = os.path.join(holdout_dir, "price3d_feature_registry.json")
    ma_path = os.path.join(holdout_dir, "price3d_moving_average_baselines.json")
    leaderboard_path = os.path.join(holdout_dir, "price3d_leaderboard.json")
    experiment_path = os.path.join(holdout_dir, "price3d_focused_experiment.json")
