...
from scripts.run_price1w_focused_experiment import compute_moving_average_baselines
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a targeted 1W search for weekly price MAPE improvements.")
    ...
    parser.add_argument(
        "--output-root",
        default=os.path.join("models", "holdout_price_1w_experiments", "round_search"),
        help="Directory root for 1W search artifacts.",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional list of experiment names to run. Defaults to the full 1W search set.",
    )
    return parser.parse_args()


def compact_registry(manager: Price1WRegressionManager, df, default_registry: Dict[str, List[str]]) -> Dict[str, Dict[str, List[str]]]:
...
def score_entry(entry: Dict[str, object], ma_best: Dict[str, object]) -> Dict[str, object]:
    ...
    passes_promotion = bool(
        ...
        and (
            improvement_absolute >= Price1WRegressionManager.PROMOTION_ABSOLUTE_THRESHOLD
            or (improvement_relative is not None and improvement_relative >= Price1WRegressionManager.PROMOTION_RELATIVE_THRESHOLD)
        )
    )
    return {
...
def main() -> None:
    ...
    manager = Price1WRegressionManager(holdout_root=output_root)
    ...
    print(f"Running {total} 1W search experiments with max {args.epochs} epochs.")
    ...
    print(f"\nSaved 1W search artifacts to {holdout_dir}")
