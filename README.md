...
## Integrated Product Walkthrough + Operator Guide

The screens below are both the product tour and the recommended operating order.
Use them top-to-bottom for a full run from data intake to portfolio execution.
...
### End-To-End Sequence (Quick Reference)

1. Complete `Quickstart (Local)` and prepare local CSV data contracts.
2. Run Screen `1` + `2` to refresh data and pass diagnostics.
3. Run Screen `3` to generate features and validate signal behavior.
4. Use Screens `4` + `5` + `6` for predictive diagnostics and regime/sensitivity checks.
5. Use Screen `11` for holdout prep and DL training.
6. Use Screen `10` to generate winner ensembles from holdout metrics.
7. Use Screen `9` to tune portfolio strategy combinations.
8. Use Screen `12` to run portfolio simulation and inspect execution ledger/KPIs.
9. Use Screens `7` + `8` for recurring automation and sequential retraining operations.
10. Before public sharing, run the checks in `Validation Commands` and `Public Artifact Policy`.
